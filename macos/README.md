...
Closing the app during installation does not silently leave the helper running. Use **Остановить** and wait for process exit. Cancelling extraction can leave an incomplete chain; reinstall successfully before opening the node. Retry starts the existing Go engine again; this is not a promise of crash-atomic snapshot extraction. The macOS flow downloads and verifies the archive before removing old chain data. A per-user lock rejects another simultaneous desktop installation. App replacement uses staging and restores the previous application on replacement failure.
